...
public class LineView extends View {
    ...
    private ArrayList<String> preTipList = new ArrayList<String>();
    private ArrayList<String> tailTipList = new ArrayList<String>();
    ...
    public static final int SHOW_POPUPS_LAST = 3;
    public static final int SHOW_POPUPS_NONE = 4;
    ...
    public void setTailTipList(ArrayList<String> tailTipList) {
        this.tailTipList = tailTipList;
    }

    public void setPreTipList(ArrayList<String> preTipList) {
        this.preTipList = preTipList;
    }
    ...
    public LineView(Context context, AttributeSet attrs) {
        ...
        popupTextPaint.setTextSize(MyUtils.sp2px(getContext(), 10));
        ...
        if (dataLists != null && !dataLists.isEmpty()) {
            ...
            for (int k = 0; k < dataLists.size(); k++) {
                ...
                for (int i = 0; i < dataLists.get(k).size(); i++) {
                    ...
                    if (i > drawDotSize - 1) {
                        //도트리스트를 추가한다.
                        if (this.preTipList.size() == dataLists.size()) {
                            drawDotLists.get(k).add(new Dot(x, 0, x, y, dataLists.get(k).get(i), k, this.preTipList.get(k), this.tailTipList.get(k)));
                        } else {
        ...
        for (int k = 0; k < drawDotLists.size(); k++) {
            ...
            for (int i = 0; i < drawDotLists.get(k).size(); i++) {
                ...
                if (showPopupType == SHOW_POPUPS_All) {
                    drawPopup(canvas, d, d.setupPoint(tmpPoint), popupColorArray[k % 3]);
                ...
                } else if (showPopupType == SHOW_POPUPS_LAST) {
                    if (i == drawDotLists.get(k).size() - 1) {
                        drawPopup(canvas, d, d.setupPoint(tmpPoint), popupColorArray[k % 3]);
                    }
                }
            }
...
// 선택한 dot 만 popup 이 뜨게 한다.
     ...
     *               0 1 2 3 4 5
     */
    private void drawPopup(Canvas canvas, Dot dot, Point point, int PopupColor) {
        String text = dot.preTip + new DecimalFormat("##.#").format(dot.data / 100.0) + dot.tailTip;

        boolean singularNum = (text.length() == 1);
        ...
        popup.setBounds(r);
        popup.draw(canvas);
        canvas.drawText(text, x, y - bottomTriangleHeight - popupBottomMargin, popupTextPaint);
    }
...
//        mViewHeight = MeasureSpec.getSize(measureSpec);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (pointToSelect != null) {
                this.showPopupType = SHOW_POPUPS_NONE;
    ...
    class Dot {
        ...
        Dot(int x, int y, int targetX, int targetY, Integer data, int linenumber, String preTip, String tailTip) {
            this.x = x;
            this.y = y;
