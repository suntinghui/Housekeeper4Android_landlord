...
public class CityPicker extends LinearLayout {
	...
	public void setSelect(String code) {
		...
		String provinceStr = code.subSequence(0, 2) + "0000";
		String cityStr = code.subSequence(0, 4) + "00";
		String countrytr = code;

		provincePicker.setData(citycodeUtil.getProvince(province_list));
		int i = 0;
		for (i = 0; i < province_list.size(); i++) {
			if (province_list.get(i).getId().equals(provinceStr)) {
		...
		int j = 0;
		cityPicker.setData(citycodeUtil.getCity(city_map, citycodeUtil.getProvince_list_code().get(i)));
		List<Cityinfo> t_city = city_map.get(provinceStr);
		for (j = 0; j < t_city.size(); j++) {
			if (t_city.get(j).getId().equals(cityStr)) {
		...
		List<Cityinfo> t_couny = couny_map.get(cityStr);
		counyPicker.setData(citycodeUtil.getCouny(couny_map, citycodeUtil.getCity_list_code().get(j)));
		for (int k = 0; k < t_couny.size(); k++) {
			if (t_couny.get(k).getId().equals(countrytr)) {
