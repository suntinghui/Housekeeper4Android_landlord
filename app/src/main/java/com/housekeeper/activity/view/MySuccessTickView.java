...
public class MySuccessTickView extends View {
	private float mDensity = -1;
	private Paint mPaint;
	private final float CONST_RADIUS = dip2px(1.2f);
	private final float CONST_RECT_WEIGHT = dip2px(2);
	private final float CONST_LEFT_RECT_W = dip2px(10);
	private final float CONST_RIGHT_RECT_W = dip2px(16);
	...
	public MySuccessTickView(Context context) {
		super(context);
		init();
	}

	public MySuccessTickView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	private void init() {
		mPaint = new Paint();
		mPaint.setColor(getResources().getColor(R.color.orangeme));
